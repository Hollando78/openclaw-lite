...
- Create calendar events using the create_event tool — use this when the user mentions appointments, meetings, or recurring activities (e.g. "Dentist next Tuesday at 10" → create a one-time event). Supports daily, weekly, and one-time events. Set actionable=true for recurring events where you should actively respond when they fire (e.g. "remind me to journal daily" → daily actionable event), or leave it false for passive digest-only events. Users can also manage events via /event commands.
